...
% In a LaTeX report, \includegraphics[width=\linewidth]{jack-monitor-states.pdf}.
%
...
\begin{tikzpicture}
  ...
  \node[info, anchor=west] at ([xshift=4pt]twoWire.east) {\hi{T} \lo{S}, stereo \lo{R} too; only T read};
  ...
  \node[state=detect, text width=3.4cm, minimum height=1.5cm] (plugcheck) at (3.4, 4.6)
    {\st{CONFIRM PLUG}\\ \sd{\lo{T} \hi{TN}, read TN}\\ \sd{is a plug in at all?}};
  ...
  \draw[trans] (16.0, 6.2) -| (rheostat.south)
    node[lbl, pos=0.3, above] {end stop, total changed $>$\,20\,\% since the pedal's first (mono rheostat)};
  ...
  \draw[trans] (plugcheck.east |- 0, 4.4) -- (open.west)
    node[lbl, pos=0.35, above] {TN $>$ \textthreequarters{} rail: a plug, but nothing behind it;\\[-1pt]forget the pedal (it may be swapped at the cable's end)};
  ...
  \node[lbl, text=ident, anchor=south] at (24.9, 4.4) {tip or ring conducts, or 1\,s passed: solve};
  \draw[reident] (27.3, 8.7) -- (27.95, 8.7);
  \node[lbl, text=ident, anchor=east] at (27.25, 8.7) {ring changed (every 100\,ms): mono ring $\neq$ sleeve,\\[-1pt]stereo ring (\lo{}) carries current};
