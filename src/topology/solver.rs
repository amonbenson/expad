use crate::adc::{AdcChain, AdcChainError};
use crate::quadbuf::{QuadBufferChain, QuadBufferChainError, TriState};
...
impl<'d, const N_QUADBUFS: usize, const N_ADCS: usize> ResistanceSolver<'d, N_QUADBUFS, N_ADCS> {
    pub fn new(config: ResistanceSolverConfig, quadbufs: QuadBufferChain<'d, N_QUADBUFS>, adcs: AdcChain<'d, N_ADCS>) -> Self {
        ResistanceSolver {
            config,
            quadbufs,
            adcs,
    ...
    pub async fn solve(&mut self, arms: [ArmConfig; 3]) -> Result<ArmResistances, ResistanceSolverError> {
        ...
        if first_measurement.conducts && second_measurement.conducts {
            ...
            self.resolve_from_single_pair(first_measurement, [0.0, 0.0, f32::INFINITY], [f32::NAN, f32::NAN, f32::INFINITY])
        } else if second_measurement.conducts {
            // Arm 1 is infinite (isolated)
            self.resolve_from_single_pair(second_measurement, [0.0, f32::INFINITY, 0.0], [f32::NAN, f32::INFINITY, f32::NAN])
        } else {
            ...
            if third_measurement.conducts {
                ...
                self.resolve_from_single_pair(third_measurement, [f32::INFINITY, 0.0, 0.0], [f32::INFINITY, f32::NAN, f32::NAN])
            } else {
                // All three arms are isolated from each other ("nothing connected")
                Ok(ArmResistances { relative: [f32::INFINITY, f32::INFINITY, f32::INFINITY], total: f32::NAN })
            }
        }
    }

    fn resolve_from_both_pairs(&self, first_measurement: PairMeasurement, second_measurement: PairMeasurement) -> Result<ArmResistances, ResistanceSolverError> {
        let first_pair_shorted = first_measurement.pair_resistance < self.config.short_resistance_threshold;
        let second_pair_shorted = second_measurement.pair_resistance < self.config.short_resistance_threshold;

        if first_pair_shorted && second_pair_shorted {
            return Ok(ArmResistances { relative: [0.0; 3], total: 0.0 });
        }

        if first_pair_shorted {
            // Arms 0 and 1 are shorted, so arm 2 holds all the resistance
            return Ok(ArmResistances { relative: [0.0, 0.0, 1.0], total: second_measurement.pair_resistance });
        }

        if second_pair_shorted {
            // Arms 0 and 2 are shorted, so arm 1 holds all the resistance
            return Ok(ArmResistances { relative: [0.0, 1.0, 0.0], total: first_measurement.pair_resistance });
        }

        let second_arm_resistance = first_measurement.low_resistance_fraction * first_measurement.pair_resistance;
        let third_arm_resistance = second_measurement.low_resistance_fraction * second_measurement.pair_resistance;
        let shared_arm_resistance_from_first_measurement = (1.0 - first_measurement.low_resistance_fraction) * first_measurement.pair_resistance;
        let shared_arm_resistance_from_second_measurement = (1.0 - second_measurement.low_resistance_fraction) * second_measurement.pair_resistance;
        ...
        let shared_arm_resistance_sum = shared_arm_resistance_from_first_measurement + shared_arm_resistance_from_second_measurement;
        let shared_arm_disagreement = (shared_arm_resistance_from_first_measurement - shared_arm_resistance_from_second_measurement).abs();
        if shared_arm_disagreement / (shared_arm_resistance_sum + self.config.resistance_sum_epsilon) > self.config.resistance_consistency_threshold {
    ...
    fn resolve_from_single_pair(&self, measurement: PairMeasurement, shorted_arms: [f32; 3], unresolved_arms: [f32; 3]) -> Result<ArmResistances, ResistanceSolverError> {
        if measurement.pair_resistance < self.config.short_resistance_threshold {
            Ok(ArmResistances { relative: shorted_arms, total: f32::NAN })
        } else {
            Ok(ArmResistances { relative: unresolved_arms, total: measurement.pair_resistance })
        }
    }

    async fn measure_pair(&mut self, arms: [ArmConfig; 3], low_index: usize, high_index: usize) -> Result<PairMeasurement, ResistanceSolverError> {
        ...
        // Set the arm states
        self.quadbufs.set_output(high_arm.quadbuf_chip, high_arm.quadbuf_channel, TriState::High);
        self.quadbufs.set_output(low_arm.quadbuf_chip, low_arm.quadbuf_channel, TriState::Low);
        self.quadbufs.set_output(floating_arm.quadbuf_chip, floating_arm.quadbuf_channel, TriState::HiZ);
        self.quadbufs.update().map_err(ResistanceSolverError::QuadBufferChainError)?;

        // Measure the voltage on the floating arm
        let high_voltage = self.adcs.measure_channel(high_arm.adc_chip, high_arm.adc_channel).await.map_err(ResistanceSolverError::AdcChainError)?;
        let low_voltage = self.adcs.measure_channel(low_arm.adc_chip, low_arm.adc_channel).await.map_err(ResistanceSolverError::AdcChainError)?;
        let floating_voltage = self.adcs.measure_channel(floating_arm.adc_chip, floating_arm.adc_channel).await.map_err(ResistanceSolverError::AdcChainError)?;
